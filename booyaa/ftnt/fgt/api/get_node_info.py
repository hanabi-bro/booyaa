def get_node_info(api):
    if api.fgt_info.version >= '7.2.0':
        check = api.monitor
        let = api.monitor.system_csf.get()
        if let['code'] != 0:
            let['msg'] += ' Failed get_node_info()'
            return let

        res = let['output']
        res_data = let['output']['results']['devices']['fortigate'][0]

        api.fgt_info.version = res['version'].lstrip('v')
        api.fgt_info.major, api.fgt_info.minor, api.fgt_info.patch = api.fgt_info.version.split('.')
        api.fgt_info.build = res['build']
        api.fgt_info.serial = res['serial']

        # api.hostname = res_data['state']['hostname']
        api.fgt_info.hostname = res_data['host_name']
        api.fgt_info.model = res_data['model']                      # "model": "FGT60F"

        # ha_mode_dict = {0:'Standalone', 1: 'Active-Active', 2: 'Active-Passive'}
        # api.ha_mode = ha_mode_dict[res_data['ha_mode']]
        api.fgt_info.ha_mode = res_data['ha_mode']
        api.fgt_info.ha_role = 'main' if res_data['is_ha_master'] == 1 else 'secondary'

        # セカンダリノード情報
        _ha_list = res_data['ha_list']
        if len(_ha_list) == 2:

            secondary_info = next(
                (l for l in _ha_list if l['serial_no'] != api.fgt_info.serial)
            )

            api.fgt_info.secondary_hostname = secondary_info['hostname']
            api.fgt_info.secondary_serial = secondary_info['serial_no']

        api.fgt_info.ha_mgmt_status = ''      # mgmt statusが有効ならHAは見ない
        api.fgt_info.ha_mgmt_interfaces = []  # さらに念のため見るならmgmt_interfacesが1以上


    elif api.fgt_info.version < '7.2.0':
        let = api.monitor.web_ui_state.get()
        if let['code'] != 0:
            let['msg'] += ' Failed get_node_info()'
            return let

        res = let['output']
        res_data = let['output']['results']

        api.fgt_info.version = res['version'].lstrip('v')
        api.fgt_info.major, api.minor, api.patch = api.fgt_info.version.split('.')
        api.fgt_info.build = res['build']
        api.fgt_info.serial = res['serial']

        api.fgt_info.hostname = res_data['hostname']
        api.fgt_info.model = res_data['model']                      # "model": "FGT60F"

        ha_mode_dict = {0:'Standalone', 1: 'Active-Active', 2: 'Active-Passive'}
        api.fgt_info.ha_mode = ha_mode_dict[res_data['ha_mode']]
        api.fgt_info.ha_role = 'main' if res_data['is_ha_master'] == 1 else 'secondary'

        # セカンダリノード情報
        let = api.monitor.system_ha_statistics.get()
        if let['code'] != 0:
            let['msg'] += ' Failed get_node_info()'
            return let

        _ha_list = let['output']['results']
        if len(_ha_list) == 2:

            secondary_info = next(
                (l for l in _ha_list if l['serial_no'] != api.fgt_info.serial)
            )

            api.fgt_info.secondary_hostname = secondary_info['hostname']
            api.fgt_info.secondary_serial = secondary_info['serial_no']


    # Managed Switchリスト
    if api.fgt_info.version < '7.0.0':
        let = api.monitor.switch_controller_managed_switch.get()
        _list = api.monitor.switch_controller_managed_switch.msw_list
    elif '7.0.0' <= api.fgt_info.version:
        let = api.monitor.switch_controller_managed_switch_status.get()
        _list = api.monitor.switch_controller_managed_switch_status.msw_list

    api.fgt_info.msw_list = _list

    return let


__sample_7_2 = {
'action': '',
'build': 1740,
'http_method': 'GET',
'name': 'csf',
'path': 'system',
'results': {'devices': {'fortigate': [{'admin_port': 443,
                                    'appliance_info': [],
                                    'authorization_type': 'serial',
                                    'device_type': 'fortigate',
                                    'device_upgrade_summary': {'cancelled': 0,
                                                                'confirmed': 0,
                                                                'coordinating': 0,
                                                                'device-disconnected': 0,
                                                                'disabled': 0,
                                                                'done': 0,
                                                                'downloading': 0,
                                                                'failed': 0,
                                                                'final-check': 0,
                                                                'initialized': 0,
                                                                'ready': 0,
                                                                'staging': 0,
                                                                'upgrade-devices': 0},
                                    'eos': False,
                                    'fabric_approval_support': False,
                                    'fap_list': [],
                                    'fap_version': 0,
                                    'fext_list': [],
                                    'fext_version': 0,
                                    'firmware_version_branch': 1740,
                                    'firmware_version_build': 1740,
                                    'firmware_version_major': 7,
                                    'firmware_version_maturity': 'M',
                                    'firmware_version_minor': 2,
                                    'firmware_version_patch': 11,
                                    'firmware_version_release_type': 'GA',
                                    'forticloud_id': 'Confidential',
                                    'forticloud_id_list': {},
                                    'fsw_list': [{'connecting_from': '10.255.1.1',
                                                    'dhcp_snooping_supported': True,
                                                    'eos': False,
                                                    'faceplate_xml': '<model '
                                                                    'type="s224en">\n'
                                                                    '\t'
                                                                    '<portgroup '
                                                                    'id="portgroup1">\n'
                                                                    '\t\t'
                                                                    '<port '
                                                                    'id="mgmt" '
                                                                    'label="MGMT"/>\n'
                                                                    '\t'
                                                                    '</portgroup>\n'
                                                                    '\t'
                                                                    '<portgroup '
                                                                    'id="portgroup2" '
                                                                    'label="" '
                                                                    'rows="2">\n'
                                                                    '\t\t'
                                                                    '<port '
                                                                    'id="port" '
                                                                    'start="1" '
                                                                    'switch_name="fsw" '
                                                                    'num="24"/>\n'
                                                                    '\t'
                                                                    '</portgroup>\n'
                                                                    '\t'
                                                                    '<portgroup '
                                                                    'id="portgroup3" '
                                                                    'label="SFP" '
                                                                    'rows="2">\n'
                                                                    '\t\t'
                                                                    '<port '
                                                                    'id="port" '
                                                                    'start="25" '
                                                                    'switch_name="fsw" '
                                                                    'type="fiber" '
                                                                    'num="4"/>\n'
                                                                    '\t'
                                                                    '</portgroup>\n'
                                                                    '</model>\n',
                                                    'fgt_peer_intf_name': 'flink1',
                                                    'igmp_snooping_supported': True,
                                                    'image_download_progress': 0,
                                                    'is_l3': False,
                                                    'join_time': 'Wed Aug  6 '
                                                                '10:43:49 '
                                                                '2025',
                                                    'led_blink_supported': True,
                                                    'max_poe_budget': 0,
                                                    'mc_lag_supported': True,
                                                    'name': 'FSW01',
                                                    'os_version': 'S224EN-v7.4.3-build830,240422 '
                                                                '(GA)',
                                                    'ports': [{'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port1',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port2',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port3',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port4',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port5',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port6',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port7',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port8',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port9',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port10',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port11',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port12',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port13',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port14',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port15',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port16',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port17',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port18',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port19',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port20',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port21',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port22',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'full',
                                                                'fgt_peer_device_name': 'FGT60FTK20099VM6',
                                                                'fgt_peer_port_name': 'a',
                                                                'fortilink_port': True,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port23',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 1000,
                                                                'status': 'up',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'full',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port24',
                                                                'isl_peer_device_name': 'S224EPTF20005577',
                                                                'isl_peer_port_name': 'port24',
                                                                'isl_peer_trunk_name': '4EPTF20005577-0',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 1000,
                                                                'status': 'up',
                                                                'stp_status': 'disabled',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port25',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['1000full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port26',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['1000full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port27',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['1000full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port28',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['1000full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'}],
                                                    'serial': 'S224ENTF18000490',
                                                    'state': 'Authorized',
                                                    'status': 'Connected',
                                                    'type': 'physical',
                                                    'vdom': 'root',
                                                    'vlan_segment_lite_supported': False,
                                                    'vlan_segment_supported': True},
                                                    {'dhcp_snooping_supported': True,
                                                    'eos': False,
                                                    'faceplate_xml': '<model '
                                                                    'type="s224ep">\n'
                                                                    '\t'
                                                                    '<portgroup '
                                                                    'id="portgroup1">\n'
                                                                    '\t\t'
                                                                    '<port '
                                                                    'id="mgmt" '
                                                                    'label="MGMT"/>\n'
                                                                    '\t'
                                                                    '</portgroup>\n'
                                                                    '\t'
                                                                    '<portgroup '
                                                                    'id="portgroup2" '
                                                                    'label="PoE+" '
                                                                    'rows="2" '
                                                                    'poe="true">\n'
                                                                    '\t\t'
                                                                    '<port '
                                                                    'id="port" '
                                                                    'start="1" '
                                                                    'switch_name="fsw" '
                                                                    'num="12"/>\n'
                                                                    '\t'
                                                                    '</portgroup>\n'
                                                                    '\t'
                                                                    '<portgroup '
                                                                    'id="portgroup3" '
                                                                    'label="" '
                                                                    'rows="2">\n'
                                                                    '\t\t'
                                                                    '<port '
                                                                    'id="port" '
                                                                    'start="13" '
                                                                    'switch_name="fsw" '
                                                                    'num="12"/>\n'
                                                                    '\t'
                                                                    '</portgroup>\n'
                                                                    '\t'
                                                                    '<portgroup '
                                                                    'id="portgroup4" '
                                                                    'label="SFP" '
                                                                    'rows="2">\n'
                                                                    '\t\t'
                                                                    '<port '
                                                                    'id="port" '
                                                                    'start="25" '
                                                                    'switch_name="fsw" '
                                                                    'type="fiber" '
                                                                    'num="4"/>\n'
                                                                    '\t'
                                                                    '</portgroup>\n'
                                                                    '</model>\n',
                                                    'fgt_peer_intf_name': 'flink1',
                                                    'igmp_snooping_supported': True,
                                                    'image_download_progress': 0,
                                                    'is_l3': False,
                                                    'led_blink_supported': True,
                                                    'max_poe_budget': 180,
                                                    'mc_lag_supported': True,
                                                    'name': 'S224EPTF20005577',
                                                    'ports': [{'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port1',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port2',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port3',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port4',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port5',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port6',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port7',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port8',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port9',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port10',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port11',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port12',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': True,
                                                                'poe_status': 'enabled',
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port13',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port14',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port15',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port16',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port17',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port18',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port19',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port20',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port21',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port22',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port23',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port24',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['10half',
                                                                                        '10full',
                                                                                        '100half',
                                                                                        '100full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port25',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['1000full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port26',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['1000full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port27',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['1000full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'},
                                                            {'dhcp_snooping': {'untrusted': 0},
                                                                'duplex': 'half',
                                                                'fgt_peer_device_name': '',
                                                                'fgt_peer_port_name': '',
                                                                'fortilink_port': False,
                                                                'igmp_snooping_group': {'group_count': 0},
                                                                'interface': 'port28',
                                                                'isl_peer_device_name': '',
                                                                'isl_peer_port_name': '',
                                                                'isl_peer_trunk_name': '',
                                                                'mclag': False,
                                                                'mclag_icl': False,
                                                                'poe_capable': False,
                                                                'port_power': 0.0,
                                                                'power_status': 0,
                                                                'speed': 0,
                                                                'status': 'down',
                                                                'supported_port_speeds': ['1000full',
                                                                                        'auto',
                                                                                        '1000auto'],
                                                                'vlan': '_default'}],
                                                    'serial': 'S224EPTF20005577',
                                                    'state': 'Discovered',
                                                    'status': 'Idle',
                                                    'type': 'physical',
                                                    'vdom': 'root',
                                                    'vlan_segment_lite_supported': False,
                                                    'vlan_segment_supported': True}],
                                    'fsw_version': 1,
                                    'fweb_build': '',
                                    'ha_group_id': 100,
                                    'ha_group_name': 'lab_ha01',
                                    'ha_list': [{'hostname': 'FG60F-Primary',
                                                    'is_ha_primary': True,
                                                    'serial_no': 'FGT60FTK20099VM6'},
                                                {'hostname': 'FG60F-Secodary',
                                                    'is_ha_primary': False,
                                                    'serial_no': 'FGT60FTK20011089'}],
                                    'ha_mode': 'active-passive',
                                    'host_name': 'FG60F-Primary',
                                    'ipam_status': 'disable',
                                    'is_ha_master': 1,
                                    'is_vm': False,
                                    'mgmt_ip_str': '',
                                    'mgmt_port': 0,
                                    'model': 'FGT60F',
                                    'model_level': 'low',
                                    'model_name': 'FortiGate',
                                    'model_number': '60F',
                                    'model_subtype': '',
                                    'object_unification': 0,
                                    'path': 'FGT60FTK20099VM6',
                                    'saml_role': 'disable',
                                    'serial': 'FGT60FTK20099VM6',
                                    'state': {'admin_https_redirection': True,
                                                'bios_security_level': 1,
                                                'carrier_mode': False,
                                                'centrally_managed': False,
                                                'config_save_mode': 'automatic',
                                                'conserve_mode': False,
                                                'csf_enabled': False,
                                                'csf_group_name': '',
                                                'csf_object_sync_mode': 'local',
                                                'csf_sync_mode': 'default',
                                                'csf_upstream_ip': '',
                                                'date_format': 'yyyy/MM/dd',
                                                'date_format_device': 'system',
                                                'features': {'fortiextender': True,
                                                            'fortitoken-cloud-service': True,
                                                            'gui-app-detection-sdwan': False,
                                                            'gui-auto-upgrade-setup-warning': False,
                                                            'gui-cdn-usage': True,
                                                            'gui-certificates': False,
                                                            'gui-custom-language': False,
                                                            'gui-display-hostname': True,
                                                            'gui-firmware-upgrade-warning': True,
                                                            'gui-forticare-registration-setup-warning': True,
                                                            'gui-fortigate-cloud-sandbox': False,
                                                            'gui-ipv6': False,
                                                            'gui-local-out': False,
                                                            'gui-replacement-message-groups': False,
                                                            'gui-wireless-opensecurity': False,
                                                            'gui-workflow-management': False,
                                                            'switch-controller': True,
                                                            'wireless-controller': True},
                                                'fext_enabled': True,
                                                'fext_vlan_mode': False,
                                                'fips_cc_enabled': False,
                                                'fips_ciphers_enabled': False,
                                                'forced_low_crypto': False,
                                                'forticare_registration_level': 1,
                                                'fortimanager_backup_mode': False,
                                                'fortimanager_configured': False,
                                                'ha_mode': 2,
                                                'has_hyperscale_license': False,
                                                'has_local_config_revisions': True,
                                                'has_log_disk': False,
                                                'hostname': 'FG60F-Primary',
                                                'image_sign_status': 'certified',
                                                'is_dst_observed': False,
                                                'is_dst_timezone': False,
                                                'is_ha_master': 1,
                                                'lenc_mode': False,
                                                'management_vdom': 'root',
                                                'need_fs_check': False,
                                                'ngfw_mode': 'profile-based',
                                                'security_rating_result_submission': True,
                                                'security_rating_run_on_schedule': True,
                                                'snapshot_utc_time': 1754448192000,
                                                'time_zone_db_name': 'Asia/Tokyo',
                                                'time_zone_offset': -540,
                                                'time_zone_text': '(GMT+9:00) '
                                                                'Osaka, '
                                                                'Sapporo, '
                                                                'Tokyo, '
                                                                'Seoul',
                                                'usb_mode': False,
                                                'usg_mode': False,
                                                'utc_last_reboot': 1754444498000,
                                                'vdom_mode': ''},
                                    'subtree_members': [],
                                    'sync_mode': 1,
                                    'tree_has_multi_vdom': False,
                                    'upgrade_id': 0,
                                    'upgrade_path_index': 0,
                                    'upgrade_status': 'disabled',
                                    'vdom_info': {'root': {'central_nat_enabled': False,
                                                            'features': {'gui-advanced-policy': False,
                                                                        'gui-advanced-wireless-features': False,
                                                                        'gui-allow-unnamed-policy': False,
                                                                        'gui-antivirus': True,
                                                                        'gui-ap-profile': True,
                                                                        'gui-application-control': True,
                                                                        'gui-dhcp-advanced': True,
                                                                        'gui-dlp-profile': False,
                                                                        'gui-dns-database': False,
                                                                        'gui-dnsfilter': True,
                                                                        'gui-dos-policy': False,
                                                                        'gui-dynamic-routing': False,
                                                                        'gui-email-collection': False,
                                                                        'gui-explicit-proxy': False,
                                                                        'gui-file-filter': True,
                                                                        'gui-fortiap-split-tunneling': False,
                                                                        'gui-fortiextender-controller': True,
                                                                        'gui-icap': False,
                                                                        'gui-implicit-policy': True,
                                                                        'gui-ips': True,
                                                                        'gui-load-balance': False,
                                                                        'gui-local-in-policy': False,
                                                                        'gui-multicast-policy': False,
                                                                        'gui-multiple-interface-policy': False,
                                                                        'gui-object-colors': True,
                                                                        'gui-ot': False,
                                                                        'gui-policy-based-ipsec': False,
                                                                        'gui-policy-disclaimer': False,
                                                                        'gui-proxy-inspection': False,
                                                                        'gui-security-profile-group': False,
                                                                        'gui-spamfilter': False,
                                                                        'gui-sslvpn-personal-bookmarks': False,
                                                                        'gui-sslvpn-realms': False,
                                                                        'gui-switch-controller': True,
                                                                        'gui-threat-weight': True,
                                                                        'gui-traffic-shaping': True,
                                                                        'gui-videofilter': False,
                                                                        'gui-voip-profile': False,
                                                                        'gui-vpn': True,
                                                                        'gui-waf-profile': False,
                                                                        'gui-wan-load-balancing': True,
                                                                        'gui-webfilter': True,
                                                                        'gui-webfilter-advanced': False,
                                                                        'gui-wireless-controller': True,
                                                                        'gui-ztna': False},
                                                            'is_admin_type_vdom': False,
                                                            'is_lan_extension_vdom': False,
                                                            'is_management_vdom': True,
                                                            'ngfw_mode': 'profile-based',
                                                            'resolve_hostnames': True,
                                                            'transparent_mode': False,
                                                            'virtual_wire_pair_count': 0}},
                                    'vdom_mode': False,
                                    'vdoms': ['root']}]},
            'protocol_enabled': False},
'revision': '33.2.1',
'serial': 'FGT60FTK20099VM6',
'status': 'success',
'vdom': 'root',
'version': 'v7.2.11'}
