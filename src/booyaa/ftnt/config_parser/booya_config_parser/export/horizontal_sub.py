    ...
    for r, param in enumerate(config_obj, start=row + 1):
        ...
        for col, field_name in enumerate(field_names, start=0):
            if field_name == '__layout__':
                continue
            try:
                if isinstance(param[key][field_name], str):
                    cell_value = param[key][field_name]
                elif isinstance(param[key][field_name], list):
                    cell_value = '\n'.join(map(str, param[key][field_name]))
                else:
                    cell_value = str(param[key][field_name])
